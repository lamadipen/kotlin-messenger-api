...
internal object TokenAuthenticationService {
    ...
    private val AUTHORIXATION_HEADER_KEY = "Authorization"
    ...
    fun addAuthentication(res: HttpServletResponse, username: String){
        val JWT = Jwts.builder()
                ...
                .compact()
        res.addHeader(AUTHORIXATION_HEADER_KEY,"$TOKEN_PREFIX $JWT")
    }

    fun getAuthentication(request: HttpServletRequest): Authentication?{
        val token = request.getHeader(AUTHORIXATION_HEADER_KEY)
        if(token != null){
